...
def FoveateAt(img_tsr, pnt: tuple, kerW_coef=0.04, e_o=1, \
    ...
    maxecc = math.sqrt(max(xid, W - xid) ** 2 + max(yid,
                                                    H - yid) ** 2) * deg_per_pix  # max([D2fov_deg[0,0], D2fov_deg[-1,0], D2fov_deg[0,-1], D2fov_deg[-1,-1]])
    # e_r = maxecc; # 15
    if N_e is None:
...
def randomFoveated(img_tsr, pntN: int, kerW_coef=0.04, fov_area_rng=1, N_e=None, spacing=0.5, bdr=32, tfm_ver=False):
    ...
    for it in range(pntN):
        ...
        D2fov = torch.sqrt((XX - xid) ** 2 + (YY - yid) ** 2)
        D2fov_deg = D2fov * deg_per_pix
        maxecc = max(D2fov_deg[0, 0], D2fov_deg[-1, 0], D2fov_deg[0, -1],
                     D2fov_deg[-1, -1])  # maximal deviation at 4 corner
        e_r = maxecc;  # 15
        # if N_e is None:
...
def get_RandomFoveationTfm(kerW_coef=0.06, fov_area_rng=(0.01, 0.5), bdr=12):
    ...
    tfm = lambda imgtsr: randomFoveated(imgtsr, 1, kerW_coef=kerW_coef, fov_area_rng=fov_area_rng, spacing=0.2, bdr=bdr,
                                        tfm_ver=True)
    return tfm
...
def foveation(img, pnt, kerW_coef=0.04, e_o=1.0, N_e=None, spacing=0.5):
    """
    Parameters:
      img: input image(.png)
      pnt: (x,y) coordinate of the foveation point
      kerW_coef: how gaussian filtering kernel std scale as a function of eccentricity
      e_o: eccentricity of the initial ring belt
      N_e: Number of ring belts in total. if None, it will calculate the N_e s.t. the whole image is covered by ring belts.
      spacing: log scale spacing between eccentricity of ring belts.
    Returns:
      fov_views: Foveation image (NumPy array).
      """
    # img_array = np.array(img)
    # img_tsr = torch.tensor(img_array / 255.0).permute([2, 0, 1]).unsqueeze(0).float()
    img_tsr = torchvision.transforms.functional.to_tensor(img)
