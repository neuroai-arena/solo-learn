...
IMG_EXTENSIONS = ['.h5',]
# https://github.com/abdo-eldesokey/nconv-nyu/blob/master/dataloaders/dataloader.py
...
class Resize(object):
    ...
    def __init__(self, size, interpolation='nearest'):
        ...
        # self.resize = torchvision.transforms.Resize(int(self.size), InterpolationMode.NEAREST)

    def __call__(self, img):
        sy, sx = int(img.shape[0] * self.size), int(img.shape[1] * self.size)
        if img.ndim == 3:
            # return torchvision.transforms.functional.resize(img, (sx, sy), InterpolationMode.NEAREST)
            img = cv2.resize(img, (sx, sy), interpolation=cv2.INTER_NEAREST)
        elif img.ndim == 2:
            # return skimage.transform.resize(img, self.size, self.interpolation, 'F')
            # return self.resize(img)
            img = cv2.resize(img, (sx, sy), interpolation=cv2.INTER_NEAREST)
            # return torchvision.transforms.functional.resize(img, new_size, InterpolationMode.NEAREST)
        else:
...
class MyDataloader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # try:
        depth_tensor = to_tensor(depth_np.copy())
        while depth_tensor.dim() < 3:
...
class NYUDataset(MyDataloader):
    def __init__(self, root, split, sparsifier=None, modality='rgb', **kwargs):
        super(NYUDataset, self).__init__(root, split, sparsifier, modality)
    ...
    def train_transform(self, rgb, depth):
        ...
        rgb_np = np.asarray(rgb_np, dtype='float32') / 255
