...
if __name__ == "__main__":
    ...
    img = PIL.Image.open("/home/aubret/Documents/postdoc/gym_results/test_images/ego4d/540/0.png")
    imgtsr = torchvision.transforms.functional.to_tensor(img)
    # imgtsr = torch.tensor(img).permute([2, 0, 1]).float()
    # imgtsr = torch.tensor(img).permute([2, 0, 1]).float()
    # %%
    d = "/home/aubret/Documents/postdoc/gym_results/test_images/variants"
    os.makedirs(d, exist_ok=True)
    img_cm = img_cortical_magnif_tsr(imgtsr, (270, 270), linear_separable_gridfun)
    torchvision.utils.save_image(img_cm, os.path.join(d, "linar.png"))
    # %%
    img_cm = img_cortical_magnif_tsr(imgtsr, (270, 270), normal_gridfun)
    torchvision.utils.save_image(img_cm, os.path.join(d, "normal.png"))

    # %%
    img_cm = img_cortical_magnif_tsr(imgtsr, (270, 270),
                                     lambda img, pnt: radial_quad_isotrop_gridfun(img, pnt, fov=20, K=20))
    torchvision.utils.save_image(img_cm, os.path.join(d, "quad.png"))

    # %%
    img_cm = img_cortical_magnif_tsr(imgtsr, (270, 270),
                                     lambda img, pnt: radial_exp_isotrop_gridfun(img, pnt, slope_C=2.0,
                                                                                 cover_ratio=0.4))
    torchvision.utils.save_image(img_cm, os.path.join(d, "exp.png"))

    for fov in [5, 10, 20, 40, 80]:
         for k in [1, 10, 20, 50, 100]:
            img_cm = img_cortical_magnif_tsr(imgtsr, (270, 270),
                                             lambda img, pnt: radial_quad_isotrop_gridfun(img, pnt, fov=fov, K=k))
            torchvision.utils.save_image(img_cm, os.path.join(d, f"quad{fov}{k}.png"))
